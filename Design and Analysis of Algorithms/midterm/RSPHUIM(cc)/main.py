...
with open(input, "r") as file:
    ...
    for line in file:
        ...
        items = {
            item: int(qty)
            for item, qty in (pair.split(":") for pair in items.split(" "))
        }
...
def initial_scan(
    ...
    filtered_items = {
        item
        for item in item_twut
        if item_twut[item] >= min_util and item_recency[item] >= min_rec
    }
...
def construct_rspul(
    ...
    for transaction in transaction_data:
        items = [item for item in transaction["Items"] if item in filtered_items]
        for r in range(1, len(items) + 1):  # Support sets of items
            for itemset in combinations(items, r):
                utility = sum(
                    transaction["Items"][i] * external_utilities[i] for i in itemset
                )
                ...
                remaining_utility = sum(
                    ...
                    if it not in itemset
                )
                rspul[itemset].append(
                    TransactionEntry(
    ...
    for itemset, entries in rspul.items():
        ...
        if total_recency >= min_rec and max_period <= max_per:
            rsphui_candidates.append(
                (itemset, total_utility, total_recency, max_period)
            )
...
def run_rsphuim_algorithm(
    transaction_data, external_utilities, min_util=30, min_rec=1.0, max_per=5, k=10
):
...
for itemset, utility, recency, period in top_k_rsphui:
    print(
        f"Itemset: {itemset}, Utility: {utility}, Recency: {recency}, Period: {period}"
    )
