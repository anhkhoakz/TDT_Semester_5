...
# Define the transaction entry structure including periodicity
TransactionEntry = namedtuple(
    "TransactionEntry",
    ["transaction_id", "utility", "recency", "remaining_utility", "periodicity"],
)

# Manually defined dataset based on the report's example
transaction_data = [
...
# Calculate utility of each item in a transaction
def calculate_item_utility(transaction, external_utilities):
    return {
        item: qty * external_utilities[item]
...
# Calculate recency of transaction
...
# Calculate periodicity for each item
def calculate_periodicity(transactions):
    item_last_occurrence = {}
    item_periodicities = defaultdict(list)

    for transaction in transactions:
        for item in transaction["Items"]:
            if item in item_last_occurrence:
                # Calculate difference between current and last occurrence
                period = transaction["Tid"] - item_last_occurrence[item]
                item_periodicities[item].append(period)
            item_last_occurrence[item] = transaction["Tid"]
    # Take the maximum period as the periodicity for each item
    return {item: max(periods) for item, periods in item_periodicities.items()}
...
# Step 2: Construct Recent Short Period Utility List (RSPUL) with Periodicity
def construct_rspul(
    transaction_data, filtered_items, external_utilities, decay_factor=0.1
):
    rspul = defaultdict(list)
    current_date = max(transaction["Timestamp"] for transaction in transaction_data)
    item_periodicities = calculate_periodicity(transaction_data)
    ...
    for transaction in transaction_data:
        for item in transaction["Items"]:
            if item in filtered_items:
                ...
                periodicity = item_periodicities.get(item, float("inf"))
                rspul[item].append(
                    TransactionEntry(
                        transaction["Tid"],
                        utility,
...
# Step 3: Mining Top-K RSPHUIs using RSPUL and Pruning with Periodicity Constraint
def mine_rsphui(rspul, item_twut, item_recency, min_util, min_rec, max_per, k):
    rsphui_candidates = []
    ...
    for item, entries in rspul.items():
        ...
        max_periodicity = max(entry.periodicity for entry in entries)

        # Apply utility, recency, and periodicity constraints
        if (
            total_utility >= min_util
            and total_recency >= min_rec
            and max_periodicity <= max_per
        ):
            rsphui_candidates.append(
                (item, total_utility, total_recency, max_periodicity)
            )
...
def run_rsphuim_algorithm(
    transaction_data, external_utilities, min_util=100, min_rec=1.0, max_per=10, k=5
):
    ...
    top_k_rsphui = mine_rsphui(
        rspul, item_twut, item_recency, min_util, min_rec, max_per, k
    )
...
# Run the algorithm
top_k_rsphui = run_rsphuim_algorithm(transaction_data, external_utilities, k=1)
print("Top-K Periodic High Utility Itemsets:")
for item, utility, recency, periodicity in top_k_rsphui:
    print(
        f"Item: {item}, Utility: {utility}, Recency: {recency}, Periodicity: {periodicity}"
    )
