...
for i in range(1, num_transactions + 1):
    ...
    item_quantities_str = ",".join(
        [f"{item}:{quantities[item]}" for item in transaction_items]
    )
    transaction_data.append(
        [i, timestamp.strftime("%Y-%m-%d %H:%M"), item_quantities_str]
    )

# Save to CSV
transactions_df = pd.DataFrame(
    transaction_data, columns=["TransactionID", "Timestamp", "Item_Quantities"]
)
